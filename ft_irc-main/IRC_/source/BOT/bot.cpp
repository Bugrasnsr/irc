...
{
	return this->bot_file_descriptor;
} */
...
void Bot::createBotSocket(void)
{
	...
	/* if (fcntl(this->bot_file_descriptor, F_SETFL, O_NONBLOCK) < 0)
	{
...
void Bot::runBot(void)
{
	...
	while (1)
	{
		...
		if(cliMsg.empty())
			continue;
		MessageAnalyzer(cliMsg);
	}
}

void Bot::MessageAnalyzer(const std::string &message)
{
	std::string target = splitByFirstSpace(message)[0];
